...
// Ensure font registration with error handling
...
// Add roundRect polyfill for older canvas versions
if (canvasAvailable) {
  try {
...
async function getTargetUser(api, event, args) {
  ...
  if (event.messageReply) {
    ...
    try {
      ...
      if (!ThreadInfo || !ThreadInfo.userInfo) return null;

      const all = ThreadInfo.userInfo;
      const botID = api.getCurrentUserID();
      let candidates = all.filter(u => u.id !== event.senderID && u.id !== botID);

      if (candidates.length === 0) return null;

      // Get current user's gender for gender-based pairing
      try {
        const currentUserInfo = await api.getUserInfo(event.senderID);
        const currentUserGender = currentUserInfo[event.senderID]?.gender;
        ...
        if (currentUserGender) {
          ...
          for (const candidate of candidates) {
            try {
              const candidateInfo = await api.getUserInfo(candidate.id);
              const candidateGender = candidateInfo[candidate.id]?.gender;
              
              // Pair opposite genders: 1 = female, 2 = male
              if ((currentUserGender === 1 && candidateGender === 2) || 
                  (currentUserGender === 2 && candidateGender === 1)) {
                oppositeGenderCandidates.push(candidate);
              }
            } catch (error) {
              // If can't get gender info, skip this candidate
              continue;
            }
          }

          // Use opposite gender candidates if available, otherwise fall back to all candidates
          if (oppositeGenderCandidates.length > 0) {
            candidates = oppositeGenderCandidates;
          }
        }
      } catch (error) {
        // If gender detection fails, continue with all candidates
        console.log("Gender detection failed:", error.message);
      }
...
module.exports = {
  config: {
    ...
    author: "Christus",
    role: 0,
    description: canvasAvailable ? "Create romantic pair images with gender-based matching" : "Canvas module not available",
    category: "media",
    guide: {
      en: canvasAvailable ? 
        "pair - Random gender-based pair from group\n"
        + "pair @mention - Pair with mentioned user\n"
        + "pair <uid> - Pair with user by ID\n"
        + "pair <theme> - Use specific theme\n"
        + "Available themes: paradise, cosmic, enchanted, royal, sunset, ocean"
        : "Canvas module is not installed. Run 'npm install canvas' to use this command."
    }
  },

  onStart: async function ({ api, event, args, usersData, threadsData }) {
    if (!canvasAvailable) {
      return api.sendMessage("❌ Canvas module is not installed. Please install the canvas dependency to use this command.", event.threadID, event.messageID);
    }

    try {
      const processingMsg = await api.sendMessage("💫 Creating your romantic pair...", event.threadID);

      const themeNames = Object.keys(romanticThemes);
      let selectedTheme = "paradise";

      if (args[0] && themeNames.includes(args[0].toLowerCase())) {
      ...
      let pathImg = path.join(cacheDir, "magical_romance.png");
      let pathAvt1 = path.join(cacheDir, "avatar1.png");
      let pathAvt2 = path.join(cacheDir, "avatar2.png");
      ...
      if (!id2) {
        ...
        return api.sendMessage("❌ No suitable user found for pairing. Try mentioning someone or ensure there are other users in the group!", event.threadID, event.messageID);
      }
      ...
      if (!userInfo1[id1] || !userInfo2[id2]) {
        await api.unsendMessage(processingMsg.messageID);
        return api.sendMessage("❌ Unable to fetch user information. Please try again!", event.threadID, event.messageID);
      }
      ...
      fs.writeFileSync(pathAvt1, Buffer.from(avatar1Data.data));
      fs.writeFileSync(pathAvt2, Buffer.from(avatar2Data.data));
      ...
      return api.sendMessage(`❌ An error occurred while creating your magical romantic pair: ${err.message}`, event.threadID, event.messageID);
    }
  },
};
